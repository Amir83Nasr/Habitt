...
Provides prompt loops that only return valid user input.
"""
...
def prompt_shamsi_date(prompt_text: str, default_today: bool = False) -> str:
    """Loop until user enters a valid Shamsi date or uses default (today)."""
    while True:
        ...
        if not user_input.strip():
            ...
            print("[red]Date cannot be empty.[/red]")
            continue
...
def prompt_time(prompt_text: str, default: str = "") -> str:
    ...
    while True:
        ...
        if not user_input.strip():
            if default:
                return default
            print("[red]Time cannot be empty.[/red]")
            continue
        try:
            ...
            print("[red]Invalid time format. Use HH:MM (e.g., 07:09 or 7:9).[/red]")
